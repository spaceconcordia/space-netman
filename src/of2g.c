...
bool of2g_valid_frame(of2g_frame_t * frame)
{
	...
	// verify checksum
	SC_checksum frame_checksum = SC_fletcher16(frame, length+3);
...
of2g_frametype_t of2g_get_frametype(of2g_frame_t * frame)
{
	...
	uint8_t fid = of2g_get_fid(frame);
	uint8_t ackid = of2g_get_ackid(frame);
...
bool of2g_build_data_frame(uint8_t * buffer, uint8_t length, uint8_t fid, of2g_frame_t * out)
{
	...
	*(out[2]) = length + 5;
	...
	// place checksum
	SC_checksum frame_checksum = SC_fletcher16(out, length+3);	// TODO: fix checksum for int
...
bool of2g_build_ack_frame(of2g_frame_t * data_frame, of2g_frame_t * out)
{
	...
	*(out[2]) = 5; // length of ACK is 5 assuming no data
	...
	// place checksum
	SC_checksum frame_checksum = SC_fletcher16(out, 3);
